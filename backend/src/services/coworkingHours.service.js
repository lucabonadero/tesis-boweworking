...
export function validarDiaReservaNoEnElPasado(diaYmd) {
  ...
  const today = fechaCalendarioEnZona();
  if (dr < today) return "No se pueden crear o modificar reservas en fechas pasadas.";
  return null;
}

/**
 * Para turnos el mismo día, el inicio no puede ser anterior a la hora actual (zona coworking).
 * @param {unknown} diaYmd
 * @param {unknown} horaIni HH:MM
 */
...
/**
 * Valida que un turno quede dentro de [09:00, 21:00] (fin inclusive hasta 21:00).
 */
export function validarVentanaOperativaTurno(horaInicio, horaFin) {
  const open = minutosDesdeMedianoche(COWORKING_APERTURA);
  const close = minutosDesdeMedianoche(COWORKING_CIERRE);
  ...
  if (hi < open) return "El horario de apertura es a las 09:00 hs.";
  if (hf > close) return "La reserva no puede finalizar después de las 21:00 hs.";
